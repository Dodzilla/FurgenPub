...
function provisioning_start() {
    ...
    if [[ "${ASSET_GEN_V5_LITE_VERIFY_LTX_REFERENCE_AUDIO,,}" == "true" ]]; then
        ...
        printf "Skipping LTX reference-audio verification for asset_gen_v5_lite.\n"
    fi
...
function dependency_manager_persist_agent_env() {
    ...
    for key in \
        ...
        GPU_ADMISSION_MODE \
        DM_GPU_ADMISSION_MAX_DEPTH \
        DM_GPU_ADMISSION_TICKET_STALE_SECONDS \
        DM_GPU_ADMISSION_RECOVERY_SECONDS \
        DM_GPU_ADMISSION_HEARTBEAT_SECONDS \
        HF_TOKEN \
        CIVITAI_TOKEN \
        COMFYUI_ARGS \
        COMFY_NODE_PINS \
