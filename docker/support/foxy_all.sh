...
ASSET_GEN_V5_DEFAULT_BOOTSTRAP_BUNDLES="${ASSET_GEN_V5_DEFAULT_BOOTSTRAP_BUNDLES:-${ASSET_GEN_V5_DEFAULT_BOOTSTRAP_BUNDLE:-asset_gen_v6_lite_comfy_core_v0251,asset_gen_v6_lite_runtime_helpers,asset_gen_v5_runtime_helpers,asset_gen_v5_flux_image,asset_gen_v5_omnivoice,asset_gen_v5_audio_annotation,asset_gen_v5_ltx23_fp8,video_gen_v2_10s_ltx_nodes}}"
...
ASSET_GEN_V5_LITE_INSTALL_VIDEO_TOOLS="${ASSET_GEN_V5_LITE_INSTALL_VIDEO_TOOLS:-false}"

TRELLIS2_ENABLE="${TRELLIS2_ENABLE:-false}"
...
function append_bundle_repos() {
    local bundle_id="$1"
    case "$bundle_id" in
        asset_gen_v6_lite_comfy_core_v0251)
            ;;
        asset_gen_v6_lite_runtime_helpers)
        ...
        asset_gen_v5_nvidia_rtx_nodes)
            ...
            ;;
        asset_gen_v5_vfi_nodes)
            append_unique_node_repo "https://github.com/GACLove/ComfyUI-VFI"
            ;;
        *)
            printf "ERROR: Unknown foxy_all bundle id '%s'.\n" "$bundle_id"
            return 1
            ;;
    esac
}
...
function provisioning_default_bundle_plan() {
    ...
    if [[ ${#SELECTED_NODE_BUNDLE_IDS[@]} -eq 0 ]]; then
        SELECTED_NODE_BUNDLE_IDS=("asset_gen_v6_lite_comfy_core_v0251" "asset_gen_v6_lite_runtime_helpers" "asset_gen_v5_runtime_helpers" "asset_gen_v5_flux_image")
    fi
}
...
function provisioning_fetch_bootstrap_bundle_plan() {
    ...
    if [[ -z "${FCS_API_BASE_URL:-}" || -z "${DEPENDENCY_MANAGER_SHARED_SECRET:-}" || -z "${DM_INSTANCE_ID:-}" ]]; then
        printf "WARN: foxy_all bootstrap plan request missing env (FCS_API_BASE_URL/DEPENDENCY_MANAGER_SHARED_SECRET/DM_INSTANCE_ID); using default bundles %s.\n" "${ASSET_GEN_V5_DEFAULT_BOOTSTRAP_BUNDLES}"
    ...
    response="$(
        ...
        printf "WARN: foxy_all bootstrap plan request failed; using default bundles %s.\n" "${ASSET_GEN_V5_DEFAULT_BOOTSTRAP_BUNDLES}"
        provisioning_default_bundle_plan
        return 0
    }
...
PY
    ...
    printf "foxy_all bootstrap bundles: %s\n" "${SELECTED_NODE_BUNDLE_IDS[*]}"
}
...
function provisioning_install_selected_node_bundles() {
    ...
    if [[ ${#NODES[@]} -gt 0 ]]; then
        ...
        printf "No foxy_all node bundles selected; skipping custom-node clone/install phase.\n"
    fi

    if [[ "${ASSET_GEN_V5_LITE_INSTALL_VIDEO_TOOLS,,}" == "true" ]] || \
        bundle_selected "asset_gen_v5_ltx23_fp8" || \
        bundle_selected "video_gen_v2_10s_ltx_nodes" || \
        bundle_selected "asset_gen_v5_10s_ltx_tiled_nodes"; then
        provisioning_install_furgen_video_tools_node || return 1
    else
        printf "Skipping FurgenVideoTools install for foxy_all.\n"
    fi
    if [[ "${ASSET_GEN_V5_INSTALL_MODE}" == "legacy_all" ]] || bundle_selected "asset_gen_v5_runtime_helpers"; then
        ...
        printf "Skipping FurgenAceStepRuntime install for foxy_all.\n"
    fi
...
function provisioning_update_comfyui() {
    if [[ -z "${COMFYUI_PIN_COMMIT}" ]]; then
        printf "Using ComfyUI from the base image; no ComfyUI pin requested for foxy_all.\n"
        return 0
    fi
...
function provisioning_start() {
    ...
    if [[ "${ASSET_GEN_V5_LITE_VERIFY_LTX_REFERENCE_AUDIO,,}" == "true" ]]; then
        provisioning_verify_ltx_reference_audio_support || return 1
    else
        printf "Skipping LTX reference-audio verification for foxy_all.\n"
    fi
    ...
    provisioning_repair_torch_cuda_import || true
    provisioning_configure_shared_library_paths || true
    printf "Skipping Trellis2 model downloads in foxy_all provisioning.\n"
...
case "${command}" in
    install-bundles)
        ...
        provisioning_install_requested_bundles "$@" || {
            echo "ERROR: foxy_all bundle installation failed."
            exit 1
        }
        ;;
    start|"")
